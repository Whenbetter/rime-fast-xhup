...
function reduce_emoji.func(input, env)
    ...
    local emoji_pos = config:get_int("emoji_reduce/idx") or 6

    for cand in input:iter() do
        local cand_text = cand.text:gsub(" ", "")
        local cand_text_length = string.utf8_len(cand_text)
        local prev_cand_text_length = string.utf8_len(prev_cand_text)
        ...
        then
            if (prev_cand_text_length >= 2) and (#emoji_cands >= #prev_cand_texts)
                and (cand_text_length == prev_cand_text_length)
            then
        ...
        elseif (top_cand_cnt <= emoji_pos) then
            ...
            if (#emoji_tab > 1) and (emoji_tab[1] == cand_text) then
                ...
                table.insert(prev_cand_texts, prev_cand_text)
            end
        else
            table.insert(other_cands, cand)
        end
